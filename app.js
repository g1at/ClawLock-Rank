...
function renderLeaderboard(entries) {
  ...
  if (!entries.length) {
    podium.innerHTML = '<div class="state-container"><div style="font-size:3rem;margin-bottom:10px;opacity:.5">🏆</div><div>榜单暂无成绩，快来体检加固吧！</div></div>';
    rows.innerHTML = '<div class="state-container short state-card-empty state-list-empty"><div>更多成绩正在汇聚中</div></div>';
    return;
  }
